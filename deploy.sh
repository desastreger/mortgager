...
#
# Domain: set with the MORTGAGER_DOMAIN env var, e.g.:
#   MORTGAGER_DOMAIN=mortgager.example.com ./deploy.sh --caddy
#
# Or edit DEFAULT_DOMAIN below once and forget about it.

set -euo pipefail

DEFAULT_DOMAIN="mortgager.example.com"
DOMAIN="${MORTGAGER_DOMAIN:-$DEFAULT_DOMAIN}"

REPO_DIR="$(cd "$(dirname "$0")" && pwd -P)"
CADDY_SNIPPET_DIR="/etc/caddy/Caddyfile.d"
CADDY_SNIPPET="$CADDY_SNIPPET_DIR/mortgager.caddy"
CADDY_TEMPLATE="$REPO_DIR/caddy/mortgager.caddy"
...
if [[ -d "$REPO_DIR/.git" ]]; then
  ...
  echo "Mortgager · $branch @ $short"
else
  echo "Mortgager · (not a git repo)"
fi
echo "  path:   $REPO_DIR"
echo "  domain: $DOMAIN"
